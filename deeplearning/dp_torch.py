...
def show_images(imgs, num_rows, num_cols, titles=None, scale=1.5):  #@save
    ...
    """一个图像里有多个子图，所以axes就是这些返回的子图，flatten把子图展开为子图数组"""
    axes = axes.flatten()
    for i, (ax, img) in enumerate(zip(axes, imgs)):
        if torch.is_tensor(img):
            ...
            # PIL图片
            ax.imshow(img)
        ax.axes.get_xaxis().set_visible(False)#应该就是设置坐标轴不可见
...
def train_ch6(net, train_iter, test_iter, num_epochs, lr, device):
    ...
    net.to(device)
    optimizer = torch.optim.SGD(net.parameters(), lr=lr)#权重衰减已经集成到优化算法中，PyTorch同时衰减权重和偏移
